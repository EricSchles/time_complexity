...
#def complete_runtime(code,ind,conditions,counter=0):
#    for condition in conditions:
        

#covers the case many for loops wrapped together
def poly_runtime(code,ind,conditions,counter=0):
    for condition in conditions:
        if not all([elem in code[ind] for elem in condition]):
            return "O(n^"+str(counter)+") complexity found on line:",ind+1
        else:
